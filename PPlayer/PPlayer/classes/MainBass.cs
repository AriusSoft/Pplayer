    ...
    public static class MainBass
    {
        ...
        public static bool InitBass(int hg)
        {
            if (!InitDefaultDevice)
            {
                ...
                if (InitDefaultDevice)
                {
                    PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\bass_aac.dll"));
                    PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\bass_ac3.dll"));
                    PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\bass_mpc.dll"));
                    PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\bassalac.dll"));
                    //PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\bassopus.dll")); // error
                    PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\bassflac.dll"));
                    PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\basswma.dll"));
                    PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\basswv.dll"));
                    PluginsHandles.Add(Bass.BASS_PluginLoad(Main.AppPath + @"plugins\bass_tta.dll"));
                    ...
                    if (Errors != 0)
                    {
                            MessageBox.Show(Errors + " plugins didn't download", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        Errors = 0;
                    }
                }
            }
        ...
        public static bool NextTrack()
        {
            ...
            {
                if (Main.Files.Count > Main.CurTrackNumber + 1)
                {
                    Play(Main.Files[++Main.CurTrackNumber], Volume);
                    EndOfPlaylist = false;
                    return true;
                }
                else
